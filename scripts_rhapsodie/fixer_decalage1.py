...
import os
import argparse
from corpus_dico_tiret import dico_decomposition

'''Ce script n'utilise pas le script exporter_obtenir_conllu.py parce que la gestion des fichiers
ne se fait pas avec grewpy. On traite le conllu comme n'importe quel fichier'''
        ...
        for ligne in liste:
            ...
            for key, value in dico_decomposition.items():
                if key in ligne and ligne[0] != "#":
                    lignes_modifiees.extend([ligne]*len(value))
                    cle_presente = True
                    break
            if not cle_presente:
